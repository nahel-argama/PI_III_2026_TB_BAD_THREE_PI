...
class WishlistItemNestedRouteTestCase(TestCase):
    ...
    def test_list_items_for_retailer_wishlist(self):
        response = self.client.get("/api/wishlists/items/")
    ...
    @patch("wishlist_item.views.product_client.get_product_by_id_validated")
    def test_create_item_on_wishlist(self, mocked_get_product):
        ...
        response = self.client.post(
            "/api/wishlists/items/", {"product_external_key": "external-3"}
        )
        ...
        self.assertTrue(
            WishlistItem.objects.filter(
                wishlist=self.wishlist, product_external_key="external-3"
            ).exists()
        )
    ...
    def test_delete_item_does_not_remove_image(self):
        ...
        response = self.client.delete(f"/api/wishlists/items/{self.item.id}/")
