...
class ProductAPITestCase(TestCase):
    ...
    def test_producer_can_create_product_for_self(self, mock_get_product):
        ...
        response = self.client.post(
            self.url, self.product_payload(producer=self.other_producer.user.id)
        )
    ...
    def test_create_product_uses_external_id_to_store_name(self, mock_get_product):
        ...
        response = self.client.post(
            self.url, self.product_payload(external_id="external-1")
        )
    ...
    def test_create_product_rejects_name_payload(self):
        ...
        response = self.client.post(
            self.url, self.product_payload(name="Should Not Be Accepted")
        )
    ...
    def test_retailer_lists_only_active_products(self):
        ...
        self.assertEqual(
            products_by_id[self.product.id]["producer_name"], "Producer One Farm"
        )
        self.assertEqual(
            products_by_id[self.other_product.id]["producer_name"], "Producer Two Farm"
        )
    ...
    def test_producer_cannot_update_product_from_another_producer(self):
        ...
        response = self.client.patch(
            f"{self.url}{self.other_product.id}/", {"price": "5.00"}
        )
    ...
    def test_producer_can_update_own_product(self):
        ...
        response = self.client.patch(
            f"{self.url}{self.product.id}/", {"price": "15.00"}
        )
    ...
    def test_update_product_uses_external_id_to_store_name(self, mock_get_product):
        ...
        response = self.client.patch(
            f"{self.url}{self.product.id}/", {"external_id": "external-1"}
        )
    ...
    def test_retailer_can_filter_active_products(self):
        ...
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            [item["id"] for item in response.data["results"]], [self.other_product.id]
        )
    ...
    def test_total_quantity_must_be_positive(self):
        ...
        response = self.client.post(self.url, self.product_payload(total_quantity=0))
    ...
    def test_reserved_quantity_cannot_be_negative(self):
        ...
        response = self.client.post(
            self.url, self.product_payload(reserved_quantity=-1)
        )
    ...
    def test_reserved_quantity_cannot_exceed_total_quantity(self):
        ...
        response = self.client.post(
            self.url, self.product_payload(total_quantity=5, reserved_quantity=6)
        )
    ...
    def test_partial_update_cannot_make_reserved_greater_than_total(self):
        ...
        response = self.client.patch(
            f"{self.url}{self.product.id}/", {"total_quantity": 5}
        )
    ...
    def test_price_must_be_positive(self):
        ...
        response = self.client.post(self.url, self.product_payload(price="0.00"))
    ...
    @patch("product.gemini_client.generate_product_description")
    def test_generate_product_description(self, mock_generate_description):
        mock_generate_description.return_value = "This is a generated description."
        self.client.force_authenticate(user=self.producer_user)

        response = self.client.post(
            "/api/products/generate/description/", {"product_name": "Test Product"}
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["description"], "This is a generated description."
        )

        mock_generate_description.assert_called_once_with(product_name="Test Product")
